...
use futures_intrusive::sync::{
    Semaphore as IntrusiveSemaphore, SemaphoreReleaser as IntrusiveSemaphoreReleaser,
};
...
async fn acquire_intrusive_semaphore(sem: &IntrusiveSemaphore) -> IntrusiveSemaphoreReleaser<'_> {
    sem.acquire(1).await
}

async fn acquire_tokio_semaphore(sem: &TokioSemaphore) -> TokioSemaphorePermit<'_> {
    sem.acquire().await.unwrap()
}

fn create_maillon_semaphore(permits: usize) -> semaphore::Semaphore {
    semaphore::Semaphore::new(permits)
}

async fn acquire_maillon_semaphore(sem: &semaphore::Semaphore) -> semaphore::SemaphorePermit<'_> {
    sem.acquire().await.unwrap()
}
...
macro_rules! benchmarks {
    (
        ...
        let mut group = $c.benchmark_group(concat!($rt_name, "/", $semaphore_name));
        group.bench_function("heavy contention", |b| {
            bench!(
                b,
                $rt_setup,
                $spawn_fn,
        ...
        group.bench_function("normal contention", |b| {
            bench!(
                b,
                $rt_setup,
                $spawn_fn,
        ...
        group.bench_function("no contention", |b| {
            bench!(
                b,
                $rt_setup,
                $spawn_fn,
...
fn tokio_rt_maillon_benchmarks(c: &mut Criterion) {
    benchmarks!(
        c,
        "tokio_rt",
        tokio::runtime::Runtime::new().unwrap(),
        tokio::spawn,
        "maillon",
        create_maillon_semaphore,
        acquire_maillon_semaphore,
    );
}
